...
export function compileApi(spec,outRoot){
  ...
  const providerImport=s.operations.some(op=>op.handler==="provider")
...
app.addHook("onRequest",async(request,reply)=>{
  if(request.url!=="/health"&&request.url!=="/ready"&&request.url!=="/metrics"&&request.method!=="OPTIONS"){
    try{quota.check();}catch(error){
      if(error instanceof QuotaExceededError)return reply.code(429).send({error:{code:error.code,message:error.message,limit:error.limit}});
      throw error;
    }
  }
...
`;
  ...
  if(s.provider) fs.copyFileSync(new URL("./provider.mjs",import.meta.url),path.join(dir,"provider.mjs"));
