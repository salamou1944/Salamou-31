...
assert.throws(
  () => putIdempotency('key', 'idem', requestFingerprint({ ...body, language: 'French' }), { ok: true }),
...
const corruptModule = await import('./request-ledger.mjs?corrupt=' + Date.now());
assert.throws(() => corruptModule.getIdempotency('key', 'idem'), /Unexpected token|JSON/);

fs.rmSync(corrupt, { force: true });
assert.equal(corruptModule.getIdempotency('key', 'idem'), null);

console.log(JSON.stringify({ ok: true, idempotency: 'deduplicates-successful-retries', recovery: 'corrupt-and-missing-ledger-covered' }));
